package com.amrdeveloper.lilo.lib.bui
...
object LiloRoundFunction : LiloObject(liloFunctionType), LiloCallable {
    override fun invoke(
        ...
        val magicLenFunction = collection.getAttr(name = LiloMagicMethod.ROUND)
        if ((magicLenFunction == null) || magicLenFunction !is LiloCallable) {
            throw createLiloException(liloTypeErrorType, "`${collection}` doesn't support __round__")
        }
